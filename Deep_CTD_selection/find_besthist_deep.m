...
function [ index] = find_besthist_deep( grid_lat, grid_long, grid_dates,grid_Z, grid_maxpres, ...
    LAT, LONG, DATES, Z, latitude_large, latitude_small, longitude_large, longitude_small,...
    phi_large, phi_small, age, age_large, map_pv_use, max_casts,mindepth )


% JML 4/9/2021 only look at historical data that is in the same basin as
% the float and is deeper that mindepth
%   Note that this messes up the indexing so it has to be set back to the
%   orignal index range at the end of this function.

[bsn_num_grid]=find_sarah_basin_num(grid_long,grid_lat);
[bsn_num_float]=find_sarah_basin_num(LONG,LAT);

good_bhist=find((ismember(bsn_num_grid,bsn_num_float))&grid_maxpres' >=mindepth);

grid_Z=grid_Z(good_bhist);
grid_long=grid_long(good_bhist);
grid_lat=grid_lat(good_bhist);
grid_dates=grid_dates(good_bhist);
...
%JML 4/9/2021  set the index back to the orignal range
index=good_bhist(index);
