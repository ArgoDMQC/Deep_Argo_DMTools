...
% function remove_salinity_mapping( pn_float_dir, pn_float_name, po_system_configuration )

function update_salinity_mapping_ow_deep( pn_float_dir, pn_float_name, po_system_configuration )
%
...
source_data_filename = fullfile(  po_system_configuration.FLOAT_SOURCE_DIRECTORY,strcat( pn_float_name, po_system_configuration.FLOAT_SOURCE_POSTFIX ) );
lo_float_source_data = load( source_data_filename );
% This section inperrits over missing lat and lons only for the purpose of
% mapping
lat_temp=lo_float_source_data.LAT;
lon_temp=lo_float_source_data.LONG;

[lat_temp,lon_temp]=intrp_lat_lon_ow(lat_temp,lon_temp,110);



lo_float_source_data.LAT=lat_temp;
lo_float_source_data.LONG=lon_temp;

clear lat_temp lon_temp
...
ls_float_mapped_filename = fullfile( po_system_configuration.FLOAT_MAPPED_DIRECTORY, strcat( po_system_configuration.FLOAT_MAPPED_PREFIX, pn_float_name, po_system_configuration.FLOAT_MAPPED_POSTFIX ) );
...
for ip = 1 : length( PROFILE_NO )
  a = find( la_profile_no==PROFILE_NO(ip) ) ;
  if( isempty(a)==1 )
     missing_profile_index = [ missing_profile_index, ip ] ;
  end
end
clear a ip
% Save a file to which maps are redone to update the metadata in the NETCDF 
%    files in GUI3JML 5/21/2019--------------------
nmaps=length( missing_profile_index );

old_dir=cd(po_system_configuration.DELAYED_MODE_DIRECTORY1);
cd ..
cd(po_system_configuration.FLOAT_PLOTS_SUBDIR)
save new_ow_maps.mat missing_profile_index
cd(old_dir)
...
% update mapped data matrix by missing_profile_index --------------------
 ...
 for iprof = 1: nmaps
tic
  disp(['UPDATE_SALINITY_MAPPING: Working on profile ' num2str(iprof)])

  j = missing_profile_index( iprof ) ;
  ...
  PRES = lo_float_source_data.PRES(:,j) ;
    ...
    [ la_grid_lat, la_grid_long, la_grid_dates,la_grid_maxpres ] = get_region_ow_deep( la_wmo_numbers, pn_float_name, po_system_configuration ) ;
