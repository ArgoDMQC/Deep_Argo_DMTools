...
function [ pa_grid_lat, pa_grid_long, pa_grid_dates,pa_grid_maxpres ] = get_region_ow_deep(pa_wmo_numbers, pa_float_name, po_config_data) ;
...
% 4/12/2021 JML added pa_grid_mx_pres
pa_grid_lat   = [ ] ;
pa_grid_long  = [ ] ;
pa_grid_dates = [ ] ;
pa_grid_maxpres=[];
...
for ln_index = 1:m
     ...
     ctd_file=strcat( po_config_data.HISTORICAL_DIRECTORY, po_config_data.HISTORICAL_CTD_PREFIX, sprintf( '%4d', pa_wmo_numbers(ln_index,1)),'.mat');

%  if exist(ctd_file,'file')~=0
            %toc 
    for ntyp = 2:4 % go through columns and check to see if this data type is supposed to be loaded
        if( ~isnan(pa_wmo_numbers(ln_index,1)) & pa_wmo_numbers(ln_index,ntyp) )
            ...
            %toc 
                lo_box_data = load( strcat( po_config_data.HISTORICAL_DIRECTORY, po_config_data.HISTORICAL_CTD_PREFIX, sprintf( '%4d', pa_wmo_numbers(ln_index,1))),'dates','lat','pres','long','source');
            %toc 
            ...
            elseif ntyp == 3 % the 3rd column denotes historical data
                %lo_box_data = load( strcat( po_config_data.HISTORICAL_DIRECTORY, po_config_data.HISTORICAL_BOTTLE_PREFIX, sprintf( '%4d', pa_wmo_numbers(ln_index,1))));
                 lo_box_data = load( strcat( po_config_data.HISTORICAL_DIRECTORY, po_config_data.HISTORICAL_BOTTLE_PREFIX, sprintf( '%4d', pa_wmo_numbers(ln_index,1))),'dates','lat','pres','long','source');
            elseif ntyp == 4 % the 4th column denotes Argo data
                %lo_box_data = load( strcat( po_config_data.HISTORICAL_DIRECTORY, po_config_data.HISTORICAL_ARGO_PREFIX, sprintf( '%4d', pa_wmo_numbers(ln_index,1))));
                 lo_box_data = load( strcat( po_config_data.HISTORICAL_DIRECTORY, po_config_data.HISTORICAL_ARGO_PREFIX, sprintf( '%4d', pa_wmo_numbers(ln_index,1))),'dates','lat','long','pres','source');
		  ...
		  jj=findstr(profile,'_');
            ...
            maxpres_junk= max(lo_box_data.pres);
            ...
            pa_grid_maxpres= [ pa_grid_maxpres, maxpres_junk ];
        end
    end
...
pa_grid_maxpres=pa_grid_maxpres';
